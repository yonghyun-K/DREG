...
# Usage (examples)
# ---------------------------
# 0) Baseline (same as before; OLS; produces fig/plot_p.pdf, plot_r.pdf, ...)
#    Rscript supp_simulation_figures_SM.R --task=grid --simnum=500
#
# 1) WLS grid figures (adds *_wls.pdf variants)
...
#
# 4) Choose output format (default is PDF; pass --format=png to revert to PNG):
#    Rscript supp_simulation_figures_SM.R --task=K --format=png --simnum=500
#
# ---------------------------
# Outputs (default outdir=fig, default format=pdf)
# ---------------------------
# Existing (grid / OLS):
#   fig/plot_p.pdf,  fig/plot_r.pdf
#   fig/plot_p2.pdf, fig/plot_r2.pdf
# Grid / WLS:
#   fig/plot_p_wls.pdf,  fig/plot_r_wls.pdf
#   fig/plot_p2_wls.pdf, fig/plot_r2_wls.pdf
# K-sensitivity:
#   fig/supp_K_stratified.pdf,  fig/supp_K_rejective.pdf
#   fig/supp_K_stratified_wls.pdf, fig/supp_K_rejective_wls.pdf
# Nonlinear + nonparam:
#   fig/supp_nonlinear_stratified.pdf, fig/supp_nonlinear_rejective.pdf
#   (and *_wls variants if --fit=wls)
# Pass --format=png to emit PNG files with the same base names.
# ============================================================
...
DEFAULTS <- list(
  ...
  outdir   = "fig",
  format   = "pdf",      # output figure format: pdf | png
  base_size = 16,
...
run_estimators_oneK <- function(
  ...
  # ----- Sample-splitting (DREG) with K folds
  perm <- sample.int(N)
  groups <- cut(seq_along(perm), breaks = K, labels = FALSE)
  SubIndex_list <- split(perm, groups)

  y_dreg_parts <- numeric(K)
  y_dreg_lasso_parts <- numeric(K)
  ...
  for (k in 1:K) {
    ...
    y_dreg_parts[k] <- drop(colSums(X[SubIndex, , drop = FALSE]) %*% beta2 +
    ...
    y_dreg_lasso_parts[k] <- drop(colSums(X[SubIndex, , drop = FALSE]) %*% beta2_L +
  ...
  y_DREG <- sum(y_dreg_parts)
  y_DREG_Lasso <- sum(y_dreg_lasso_parts)

  e_s_ss  <- e_oof[as.character(Index)]
  e_s_ssl <- e_oof_lasso[as.character(Index)]

  if (sampling == "stratified") {
    sigma_DREG   <- sqrt(quad_Omega_strata(e_s_ss,  n_h, len, st, cumn_h))
    sigma_DREG_L <- sqrt(quad_Omega_strata(e_s_ssl, n_h, len, st, cumn_h))
  } else {
    sigma_DREG   <- sqrt(var_poisson_approx(e_s_ss,  pi[Index]))
    sigma_DREG_L <- sqrt(var_poisson_approx(e_s_ssl, pi[Index]))
  }
  ...
  sigma_res <- c(
    ...
    DREG.Lasso = sigma_DREG_L
  )

  bias_proxy <- c(
    bias_proxy,
    DREG = sum(bias_parts[, 1]),
    DREG.Lasso = sum(bias_parts[, 2])
  )
...
run_grid <- function(
  ...
  est_names <- c("HT","Diff","GREG.oracle","GREG","DREG","GREG.Lasso","DREG.Lasso")
...
make_bias_rmse_plot <- function(res, show_legend = TRUE, base_size = 16, legend_position = "right") {
  ...
  keep <- c("GREG", "DREG", "GREG.Lasso", "DREG.Lasso")
...
save_fig <- function(plot_obj, path, width = 7.2, height = 4.2, dpi = 300) {
  # ggsave auto-detects device from file extension. For PDF we use cairo_pdf
  # when available for nicer text rendering; for PNG we use cairo for clean
  # antialiasing.
  ext <- tolower(tools::file_ext(path))
  args <- list(filename = path, plot = plot_obj,
               width = width, height = height, dpi = dpi, units = "in")
  if (ext == "pdf" && capabilities("cairo")) {
    args$device <- cairo_pdf
  } else if (ext == "png" && capabilities("cairo")) {
    args$type <- "cairo"
  }
  do.call(ggplot2::ggsave, args)
}
# Backward-compat alias (older code may still call save_png).
save_png <- save_fig
...
save_two_panel_fig <- function(p_left, p_right, path, width = 12.0, height = 4.4, dpi = 300) {
  ...
  ext <- tolower(tools::file_ext(path))
  if (ext == "pdf") {
    # cairo_pdf gives nicer font rendering and proper UTF-8 support;
    # fall back to pdf() if cairo is unavailable.
...
save_two_panel_png <- save_two_panel_fig
...
run_estimators_multiK <- function(
  ...
  y_DREG_byK <- numeric(length(K_vec)); names(y_DREG_byK) <- paste0("DREG.K", K_vec)
  y_DREG_L_byK <- numeric(length(K_vec)); names(y_DREG_L_byK) <- paste0("DREG.Lasso.K", K_vec)

  sig_DREG_byK <- numeric(length(K_vec)); names(sig_DREG_byK) <- paste0("DREG.K", K_vec)
  sig_DREG_L_byK <- numeric(length(K_vec)); names(sig_DREG_L_byK) <- paste0("DREG.Lasso.K", K_vec)
  ...
  for (ii in seq_along(K_vec)) {
    ...
    y_DREG_byK[ii] <- sum(y_parts)
    y_DREG_L_byK[ii] <- sum(y_parts_L)

    e_s_ss  <- e_oof[as.character(Index)]
    e_s_ssl <- e_oof_L[as.character(Index)]

    if (sampling == "stratified") {
      sig_DREG_byK[ii]   <- sqrt(quad_Omega_strata(e_s_ss,  n_h, len, st, cumn_h))
      sig_DREG_L_byK[ii] <- sqrt(quad_Omega_strata(e_s_ssl, n_h, len, st, cumn_h))
    } else {
      sig_DREG_byK[ii]   <- sqrt(var_poisson_approx(e_s_ss,  pi[Index]))
      sig_DREG_L_byK[ii] <- sqrt(var_poisson_approx(e_s_ssl, pi[Index]))
    }
  }

  y_res <- c(HT = y_HT, Diff = y_diff, GREG = y_GREG, GREG.Lasso = y_Lasso, y_DREG_byK, y_DREG_L_byK)
  sigma_res <- c(HT = sigma_HT, Diff = sigma_diff, GREG = sigma_GREG, GREG.Lasso = sigma_Lasso, sig_DREG_byK, sig_DREG_L_byK)
...
make_K_plot <- function(resK, show_legend = TRUE, base_size = 16, legend_position = "bottom") {
  ...
  df <- rbind(
    ...
    data.frame(K = K_vec, estimator = "DREG",       metric = "RMSE", value = get_split_vals("DREG.K",       K_vec, resK$rmse)),
    data.frame(K = K_vec, estimator = "DREG.Lasso", metric = "RMSE", value = get_split_vals("DREG.Lasso.K", K_vec, resK$rmse)),
    ...
    data.frame(K = K_vec, estimator = "DREG",       metric = "Bias", value = get_split_vals("DREG.K",       K_vec, resK$bias)),
    data.frame(K = K_vec, estimator = "DREG.Lasso", metric = "Bias", value = get_split_vals("DREG.Lasso.K", K_vec, resK$bias))
  )

  df$type <- ifelse(df$estimator %in% c("GREG", "GREG.Lasso"), "Baseline", "DREG")
  df$metric <- factor(df$metric, levels = c("RMSE", "Bias"))
  df$estimator <- factor(df$estimator, levels = c("GREG", "GREG.Lasso", "DREG", "DREG.Lasso"))
  ...
  gg <- ggplot(df, aes(
    ...
    geom_point(data = subset(df, metric == "RMSE" & type == "DREG"), size = 2) +
    scale_alpha_manual(values = c(Baseline = 0.55, DREG = 1), guide = "none") +
    scale_linetype_manual(values = c(RMSE = "solid", Bias = "dashed")) +
...
make_K_plot_combined <- function(resK_list, base_size = 14) {
  ...
  build_df <- function(resK, design) {
    ...
    df_rmse <- rbind(
      ...
      data.frame(design=design, K=K_vec, estimator="DREG",       metric="RMSE", value=get_split_vals("DREG.K",       K_vec, resK$rmse)),
      data.frame(design=design, K=K_vec, estimator="DREG.Lasso", metric="RMSE", value=get_split_vals("DREG.Lasso.K", K_vec, resK$rmse))
    )
    ...
    df_nbias <- rbind(
      ...
      data.frame(design=design, K=K_vec, estimator="DREG",       metric="-Bias", value=-get_split_vals("DREG.K",       K_vec, resK$bias)),
      data.frame(design=design, K=K_vec, estimator="DREG.Lasso", metric="-Bias", value=-get_split_vals("DREG.Lasso.K", K_vec, resK$bias))
    )
  ...
  df_all$estimator <- factor(df_all$estimator, levels = c("GREG","GREG.Lasso","DREG","DREG.Lasso"))
  ...
  gg <- ggplot2::ggplot(df_all, ggplot2::aes(
    ...
    ggplot2::geom_point(data = subset(df_all, metric == "RMSE" & type == "DREG"), size = 2.2) +
    ggplot2::scale_alpha_manual(values = c(Baseline = 0.55, DREG = 1), guide = "none") +
...
run_nonlinear <- function(
  ...
  final_res <- foreach::foreach(
    ...
    out$DREG.linear <- dreg_lin$est
    sig$DREG.linear <- dreg_lin$sigma
    ...
    if ("spline" %in% np_methods) {
      ...
      out$DREG.spline <- dreg_sp$est
      sig$DREG.spline <- dreg_sp$sigma
    }
    ...
    if ("rf" %in% np_methods) {
      ...
      out$DREG.rf <- dreg_rf$est
      sig$DREG.rf <- dreg_rf$sigma
    }
...
make_nonlinear_boxplot <- function(res_list, design_labels = NULL, base_size = 16, facet_scales = "fixed") {
  ...
  est_order <- c(
    "Diff",
    "GREG.linear", "DREG.linear",
    "GREG.spline", "DREG.spline",
    "GREG.rf", "DREG.rf"
  )
  ...
  pretty <- c(
    ...
    `DREG.linear` = "DREG (linear)",
    `GREG.spline` = "GREG (spline)",
    `DREG.spline` = "DREG (spline)",
    `GREG.rf`     = "GREG (RF)",
    `DREG.rf`     = "DREG (RF)"
  )
...
FIG_FORMAT <- tolower(opt$format %||% DEFAULTS$format)
if (!FIG_FORMAT %in% c("pdf", "png")) {
  stop("Unknown --format=", FIG_FORMAT, " (use pdf or png).")
}
FIG_EXT <- FIG_FORMAT     # "pdf" or "png"; appended to all output filenames
...
message(sprintf("Output: format=%s, dir=%s", FIG_FORMAT, OUTDIR))
  ...
  for (sampling in designs) {
    for (wls in wls_flags) {
      ...
      for (vary_over in vary_list) {
        ...
        fname <- sprintf("plot_%s%s%s.%s", vary_over, suffix_design, suffix_wls, FIG_EXT)
      ...
      if (all(c("p", "r") %in% names(res_by_vary))) {
        ...
        fname_pr <- sprintf("plot_pr_%s_%s.%s", sampling, fit_tag, FIG_EXT)
...
run_K_task <- function() {
  for (wls in wls_flags) {
    ...
    for (sampling in designs) {
      ...
      suffix_wls <- if (wls) "_wls" else ""
      fname <- sprintf("supp_K_%s%s.%s", sampling, suffix_wls, FIG_EXT)
    ...
    if (length(res_list) >= 2) {
      suffix_wls <- if (wls) "_wls" else ""
      fname2 <- sprintf("supp_K_both%s.%s", suffix_wls, FIG_EXT)
...
run_nonlinear_task <- function() {
  for (wls in wls_flags) {
    ...
    for (sampling in designs) {
      ...
      suffix_wls <- if (wls) "_wls" else ""
      fname <- sprintf("supp_nonlinear_%s%s.%s", sampling, suffix_wls, FIG_EXT)
    ...
    if (length(res_list) >= 2) {
      suffix_wls <- if (wls) "_wls" else ""
      fname2 <- sprintf("supp_nonlinear_both%s.%s", suffix_wls, FIG_EXT)
