...
#        - HT
#        - REG (OLS GREG)
#        - DREG (sample-split OLS GREG; K-fold cross-fit)
#        - REG.Spline / DREG.Spline (GAM spline over CRD, by crop)
#        - REG.RF / DREG.RF (random forest via ranger on Crop x CRD)
#
# Notes
...
estimate_total_dreg <- function(y, X_s, d_s, X_total, fold_id, wls = FALSE, return_resid = FALSE) {
...
estimate_total_gam_greg <- function(y, df_s, d_s, pop_df,
  ...
  if (!requireNamespace("mgcv", quietly = TRUE)) {
    warning("Package 'mgcv' is not installed. REG.Spline/DREG.Spline will be NA. Install via install.packages('mgcv').")
...
estimate_total_gam_dreg <- function(y, df_s, d_s, pop_df, fold_id,
                                   wls = FALSE, gam_k = 10, seed = 1,
                                   return_resid = FALSE) {
...
estimate_total_rf_greg <- function(y, df_s, d_s, pop_df,
  ...
  if (!requireNamespace("ranger", quietly = TRUE)) {
    warning("Package 'ranger' is not installed. REG.RF/DREG.RF will be NA. Install via install.packages('ranger').")
    resid <- rep(NA_real_, length(y))
    if (return_resid) return(list(total = NA_real_, resid = resid))
    return(NA_real_)
  }
...
estimate_total_rf_dreg <- function(y, df_s, d_s, pop_df, fold_id,
                                  wls = FALSE,
                                  num_trees = 200, mtry = NULL, min_node_size = 20,
                                  num_threads = 1, verbose = FALSE,
                                  seed = 1,
...
#   - DREG.Spline (sample-split GAM)
#   - REG.RF     (Random forest via ranger; model-assisted)
#   - DREG.RF    (sample-split RF)
#
...
for (j in seq_along(y_targets)) {
  ...
  # OLS REG / DREG (main regressors from calibration matrix)
  ...
  dreg_fit <- estimate_total_dreg(
    y, X_s = Xs_calib, d_s = d, X_total = X_total,
    fold_id = fold_id, wls = ss_wls, return_resid = TRUE
  )
  est_dreg <- dreg_fit$total
  ...
  dreg_gam_fit <- estimate_total_gam_dreg(
    y = y, df_s = data_np[, c("CropNm", "CRD_fac", "CRD_num")], d_s = d,
    pop_df = pop_np[, c("CropNm", "CRD_fac", "CRD_num", "Farms")],
    fold_id = fold_id,
    wls = ss_wls, gam_k = gam_k,
  ...
  est_dreg_spline <- dreg_gam_fit$total
  ...
  dreg_rf_fit <- estimate_total_rf_dreg(
    y = y, df_s = data_np[, c("CropNm", "CRD_fac")], d_s = d,
    pop_df = pop_np[, c("CropNm", "CRD_fac", "Farms")],
    fold_id = fold_id,
    wls = ss_wls,
  ...
  # OLS DREG: cross-fitted residuals (main.R style)
  se_dreg <- sqrt(varhat_stratified(e = dreg_fit$resid, stratum_id = stratum_id, strata_params = strata_params))

  # GAM / RF: residual linearization; DREG versions: cross-fitted residuals
  ...
  se_dreg_spline <- if (all(is.na(dreg_gam_fit$resid))) NA_real_ else
    sqrt(varhat_stratified(e = dreg_gam_fit$resid, stratum_id = stratum_id, strata_params = strata_params))
  ...
  se_dreg_rf <- if (all(is.na(dreg_rf_fit$resid))) NA_real_ else
    sqrt(varhat_stratified(e = dreg_rf_fit$resid, stratum_id = stratum_id, strata_params = strata_params))
  ...
  res_list[[j]] <- data.frame(
    ...
    DREG = as.numeric(est_dreg),
    DREG_se = as.numeric(se_dreg),

    GREG.Spline = as.numeric(est_spline),
    GREG.Spline_se = as.numeric(se_spline),

    DREG.Spline = as.numeric(est_dreg_spline),
    DREG.Spline_se = as.numeric(se_dreg_spline),

    GREG.RF = as.numeric(est_rf),
    GREG.RF_se = as.numeric(se_rf),

    DREG.RF = as.numeric(est_dreg_rf),
    DREG.RF_se = as.numeric(se_dreg_rf)
  )
}
...
estimator_cols <- c("GREG", "DREG", "GREG.Spline", "DREG.Spline", "GREG.RF", "DREG.RF")
...
# PDF output (in addition to displaying the plot interactively).
# Set save_pdf <- FALSE to disable file output entirely.
save_pdf <- TRUE
save_dir <- "fig"
pdf_width  <- 5     # inches
pdf_height <- 5

# Base text size (pt). Drives all axis/title/legend text via theme_bw(base_size = ...).
# Increase if labels look small in the PDF; decrease for a denser layout.
plot_base_size <- 16
...
if (make_plots) {
  ...
  # Optionally save the totals figure as PDF (in addition to plot pane).
  if (isTRUE(save_pdf)) {
    if (!dir.exists(save_dir)) dir.create(save_dir, recursive = TRUE)
    pdf_path <- file.path(save_dir, paste0(stateNm, ".pdf"))
    # cairo_pdf gives nicer font rendering and proper UTF-8 support;
    # fall back to the base pdf device if cairo is unavailable.
    ggplot2::ggsave(
      filename = pdf_path,
      plot     = p_totals,
      width    = pdf_width,
