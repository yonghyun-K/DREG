...
e = rnorm(N, 0, 1)
# mu = X %*% beta
mu = cbind(exp(1.25 * sin(X[,1:s])), X[,(s+1):ncol(X)]) %*% beta
y = mu + e
  ...
  for (K in seq_K) {
    ...
    for (k in 1:K) {
      ...
      y_debiased_Lasso2 = drop(colSums(X[SubIndex, beta_hat_Lasso != 0, drop = F]) %*% beta_hat_refit2 +
                                 sum((
                                   y_s1 - drop(X_s1[, beta_hat_Lasso != 0, drop = F] %*% beta_hat_refit2)
                                 ) * d1_s1))
    ...
    y_res = c(y_res,
      setNames(c(SS = y_debiased, SSLasso = y_debiased_Lasso, SSLasso2 = y_debiased_Lasso2), 
               paste(c("SSGREG", "SSLasso", "SSLasso2"), "(", K, ")", sep = "")))
    
    sigma_res = c(sigma_res,
      setNames(c(SS = sigma_Debiased, SSLasso = sigma_Debiased_Lasso, SSLasso2 = sigma_Debiased_Lasso2), 
               paste(c("SSGREG", "SSLasso", "SSLasso2"), "(", K, ")", sep = "")))
...
xtable(cbind(RB = BIAS2 / SE^2, CR = colMeans(res3)),
       digits = c(0, 4, 3),
...
boxplot(res, col = rep(c(3,4), times = c(5,6)))
